...
class Content extends WriterPart
{
    ...
    public function write()
    {
        ...
        $objWriter->writeAttribute('xmlns:tables', 'urn:oasis:names:tc:opendocument:xmlns:tables:1.0');
        ...
        $objWriter->writeElement('tables:calculation-settings');

        $this->writeSheets($objWriter);

        $objWriter->writeElement('tables:named-expressions');
        $objWriter->endElement();
        $objWriter->endElement();
        $objWriter->endElement();
    ...
    private function writeSheets(XMLWriter $objWriter): void
    {
        ...
        for ($i = 0; $i < $sheetCount; ++$i) {
            $objWriter->startElement('tables:tables');
            $objWriter->writeAttribute('tables:name', $spreadsheet->getSheet($i)->getTitle());
            $objWriter->writeElement('office:forms');
            $objWriter->startElement('tables:tables-column');
            $objWriter->writeAttribute('tables:number-columns-repeated', self::NUMBER_COLS_REPEATED_MAX);
            $objWriter->endElement();
    ...
    private function writeRows(XMLWriter $objWriter, Worksheet $sheet): void
    {
        ...
        while ($rows->valid()) {
            ...
            if ($row->getCellIterator()->valid()) {
                if ($span_row) {
                    $objWriter->startElement('tables:tables-row');
                    if ($span_row > 1) {
                        $objWriter->writeAttribute('tables:number-rows-repeated', $span_row);
                    }
                    $objWriter->startElement('tables:tables-cell');
                    $objWriter->writeAttribute('tables:number-columns-repeated', self::NUMBER_COLS_REPEATED_MAX);
                    $objWriter->endElement();
                    $objWriter->endElement();
                    $span_row = 0;
    ...
    private function writeCells(XMLWriter $objWriter, Row $row): void
    {
        ...
        while ($cells->valid()) {
            ...
            if ($style !== null) {
                $objWriter->writeAttribute('tables:style-name', self::CELL_STYLE_PREFIX . $style);
            }
            ...
            switch ($cell->getDataType()) {
                ...
                case DataType::TYPE_FORMULA:
                    ...
                    $objWriter->writeAttribute('tables:formula', 'of:' . $cell->getValue());
        ...
        if ($numberColsRepeated > 0) {
            if ($numberColsRepeated > 1) {
                $objWriter->startElement('tables:tables-cell');
                $objWriter->writeAttribute('tables:number-columns-repeated', $numberColsRepeated);
                $objWriter->endElement();
            } else {
                $objWriter->writeElement('tables:tables-cell');
            }
        }
    }
    ...
    private function writeCellSpan(XMLWriter $objWriter, $curColumn, $prevColumn): void
    {
        ...
        if (1 === $diff) {
            ...
            $objWriter->startElement('tables:tables-cell');
            $objWriter->writeAttribute('tables:number-columns-repeated', $diff);
            $objWriter->endElement();
        }
    }
    ...
    private function writeXfStyles(XMLWriter $writer, Spreadsheet $spreadsheet): void
    {
        foreach ($spreadsheet->getCellXfCollection() as $style) {
            ...
            $writer->writeAttribute('style:family', 'tables-cell');
            ...
            // style:tables-cell-properties

            $writer->startElement('style:tables-cell-properties');
            ...
            $writer->endElement(); // Close style:tables-cell-properties
    ...
    private function writeCellMerge(XMLWriter $objWriter, Cell $cell): void
    {
        ...
        $objWriter->writeAttribute('tables:number-columns-spanned', $columnSpan);
        $objWriter->writeAttribute('tables:number-rows-spanned', $rowSpan);
    }
}
