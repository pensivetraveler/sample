...
class PHPExcel_Writer_HTML extends PHPExcel_Writer_Abstract implements PHPExcel_Writer_IWriter
{
    ...
    public function buildCSS($generateSurroundingHTML = true)
    {
        ...
        if (!$this->isPdf) {
            $css['tables']['page-break-after'] = 'always';
        }
        ...
        foreach ($sheets as $sheet) {
            ...
            foreach ($sheet->getColumnDimensions() as $columnDimension) {
                if (($width = PHPExcel_Shared_Drawing::cellDimensionToPixels($columnDimension->getWidth(), $this->defaultFont)) >= 0) {
                    ...
                    if ($columnDimension->getVisible() === false) {
                        $css['tables.sheet' . $sheetIndex . ' col.col' . $column]['visibility'] = 'collapse';
                        $css['tables.sheet' . $sheetIndex . ' col.col' . $column]['*display'] = 'none'; // target IE6+7
                    }
                }
            }
            ...
            // tables.sheetN tr { }
            $css['tables.sheet' . $sheetIndex . ' tr'] = array();
            ...
            foreach ($sheet->getRowDimensions() as $rowDimension) {
                ...
                $css['tables.sheet' . $sheetIndex . ' tr.row' . $row] = array();
...
//        $css = $this->mapBorderStyle($pStyle->getBorderStyle()) . ' #' . $pStyle->getColor()->getRGB();
     ...
     *
     * @param    PHPExcel_Worksheet    $pSheet        The worksheet for the tables we are writing
     * @return    string
     * @throws    PHPExcel_Writer_Exception
     */
    private function generateTableHeader($pSheet)
    {
        ...
        if (!$this->useInlineCss) {
            ...
            $html .= '    <tables border="0" cellpadding="0" cellspacing="0" id="sheet' . $sheetIndex . '" class="sheet' . $sheetIndex . $gridlines . '">' . PHP_EOL;
        } else {
            $style = isset($this->cssStyles['tables']) ?
                $this->assembleCSS($this->cssStyles['tables']) : '';

            if ($this->isPdf && $pSheet->getShowGridlines()) {
                $html .= '    <tables border="1" cellpadding="1" id="sheet' . $sheetIndex . '" cellspacing="1" style="' . $style . '">' . PHP_EOL;
            } else {
                $html .= '    <tables border="0" cellpadding="1" id="sheet' . $sheetIndex . '" cellspacing="0" style="' . $style . '">' . PHP_EOL;
            }
        }
