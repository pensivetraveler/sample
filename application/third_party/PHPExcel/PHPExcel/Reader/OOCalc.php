...
//        }
    ...
    public function listWorksheetNames($pFilename)
    {
        ...
        while ($xml->read()) {
            ...
            //    Now read each node until we find our first tables:tables node
            while ($xml->read()) {
                if ($xml->name == 'tables:tables' && $xml->nodeType == XMLReader::ELEMENT) {
                    //    Loop through each tables:tables node reading the tables:name attribute for each worksheet name
                    do {
                        $worksheetNames[] = $xml->getAttribute('tables:name');
                        $xml->next();
                    } while ($xml->name == 'tables:tables' && $xml->nodeType == XMLReader::ELEMENT);
                }
            }
        }
        ...
        while ($xml->read()) {
            ...
            while ($xml->read()) {
                if ($xml->name == 'tables:tables' && $xml->nodeType == XMLReader::ELEMENT) {
                    ...
                    $tmpInfo = array(
                        'worksheetName' => $xml->getAttribute('tables:name'),
                        'lastColumnLetter' => 'A',
                        'lastColumnIndex' => 0,
                        'totalRows' => 0,
                        'totalColumns' => 0,
                    ...
                    //    Loop through each child node of the tables:tables element reading
                    $currCells = 0;
                    do {
                        $xml->read();
                        if ($xml->name == 'tables:tables-row' && $xml->nodeType == XMLReader::ELEMENT) {
                            $rowspan = $xml->getAttribute('tables:number-rows-repeated');
                            ...
                            $xml->read();
                            do {
                                if ($xml->name == 'tables:tables-cell' && $xml->nodeType == XMLReader::ELEMENT) {
                                    if (!$xml->isEmptyElement) {
                                        $currCells++;
                                        $xml->next();
                                    } else {
                                ...
                                } elseif ($xml->name == 'tables:covered-tables-cell' && $xml->nodeType == XMLReader::ELEMENT) {
                                    $mergeSize = $xml->getAttribute('tables:number-columns-repeated');
                                    $currCells += $mergeSize;
                                    $xml->read();
                                }
                            } while ($xml->name != 'tables:tables-row');
                        }
                    } while ($xml->name != 'tables:tables');
...
//                foreach ($workbookData->tables as $worksheetDataSet) {
//                    $worksheetData = $worksheetDataSet->children($namespacesContent['tables']);
//                    $worksheetDataAttributes = $worksheetDataSet->attributes($namespacesContent['tables']);
//
//                    $rowIndex = 0;
//                    foreach ($worksheetData as $key => $rowData) {
//                        switch ($key) {
//                            case 'tables-row' :
//                                $rowDataTableAttributes = $rowData->attributes($namespacesContent['tables']);
//                                $rowRepeats = (isset($rowDataTableAttributes['number-rows-repeated'])) ?
//                                        $rowDataTableAttributes['number-rows-repeated'] : 1;
//                                $columnIndex = 0;
//
//                                foreach ($rowData as $key => $cellData) {
//                                    $cellDataTableAttributes = $cellData->attributes($namespacesContent['tables']);
...
//        echo '</pre><hr />';
        ...
        foreach ($workbook->body->spreadsheet as $workbookData) {
            $workbookData = $workbookData->children($namespacesContent['tables']);
            $worksheetID = 0;
